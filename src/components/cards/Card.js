...
export default class Card extends Component {
  ...
  render() {
    ...
    var descHTML = desc => {
      ...
      desc = desc.replace(/initiative|furie|hâte|exaltation|niveau supérieur|niveau inférieur|létal|camoufle(r)?|camouflé(e)?(s)?|empoisonné(e)?(s)?|poison|insensible(s)?|frénésie:?|dernière volonté(:)?|dernières volontés(:)?|silence|bouclier|don du vol|(dé)?gèle|gelé(e)?(s)?|surcharge|condition:|auto:/gi, x => `<b>${x}</b>`);
