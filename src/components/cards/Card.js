...
export default class Card extends Component {
  ...
  render() {
    ...
    var descHTML = desc => {
      ...
      desc = desc.replace(/initiative|furie|hâte|exaltation|niveau supérieur|niveau inférieur|enrage(r)?(nt)?|enragé(e)?(s)?|létal|talent:?|camoufle(r)?(nt)?|camouflé(e)?(s)?|empoisonné(e)?(s)?|poison|insensible(s)?|frénésie:?|dernière volonté(:)?|dernières volontés(:)?|silence|bouclier(s)?|don du vol|(dé)?gèle(nt)?|(dé)?geler|gelé(e)?(s)?|surcharge|condition:|auto:/gi, x => `<b>${x}</b>`);
