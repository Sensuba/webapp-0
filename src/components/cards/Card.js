...
export default class Card extends Component {
  ...
  render() {
    ...
    var descHTML = desc => {
      ...
      desc = desc.replace(/initiative|furie|hâte|exaltation|niveau supérieur|niveau inférieur|enrage(r)?|enragé(e)?(s)?|enragent|létal|talent:?|camoufle(r)?|camouflé(e)?(s)?|camouflent|empoisonné(e)?(s)?|poison|insensible(s)?|frénésie:?|dernière volonté(:)?|dernières volontés(:)?|silence|bouclier(s)?|don du vol|(dé)?gèle(nt)?|(dé)?geler|gelé(e)?(s)?|surcharge|condition:|auto:/gi, x => `<b>${x}</b>`);
    ...
    if (src.cardType === "artifact" && src.mecha && !src.mechactive)
      src.mechactive = { description: "", fontSize: 1.3, overload: 0 }
    if (this.props.src.cardType === "hero") {
      ...
      src.mechactive = Object.assign(src.mechactive, {htmlDescription: descHTML(src.mechactive.description)});
    }
    ...
    case "artifact": result = <Artifact model={this.props.model} holographic={holographic} className={this.props.className} mechactive={this.props.mechactive} id={this.props.id} src={src} classColor={colorIdToClassName(this.props.src.idColor)}/>; break;
