var targets = {
	...
	player: (src, target) => target.occupied && target.card.isType("hero") ? "player" : false
};
...
class Types {
	...
	static location (value, src) {
		...
		case 'court': return src.area.court;
		...
		case 'opponent\'s court': return src.area.opposite.court;
	...
	static locations (value, src) {
		...
		case 'capsule': return [src.area.capsule];
		case 'nether': return [src.area.nether];
		case 'area': return src.area.field.tiles.concat([src.area.hand, src.area.court, src.area.deck]);
