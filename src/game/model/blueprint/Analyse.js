...
class Analyse extends Bloc {
	...
	countThisTurn (event, image) {
		...
		event.gameboard.log.logs.forEach(log => {
			if (log.type === "newturn")
				c = 0;
			if (event.check(log.type, log.src, log.data) && this.in[2](image, this.buildEvent(event.gameboard, log)))
				c++;
		})
		return c;
	}
	...
	countSincePreviousTurn (event, image) {
		...
		let c = 0;
		event.gameboard.log.logs.forEach(log => {
			if (log.type === "newturn" && log.src.no !== this.src.area.id.no)
				c = 0;
			if (event.check(log.type, log.src, log.data) && this.in[2](image, this.buildEvent(event.gameboard, log)))
				c++;
		})
	...
	countAllGame (event, image) {
		...
		return event.gameboard.log.logs.filter(log => event.check(log.type, log.src, log.data) && this.in[2](image, this.buildEvent(event.gameboard, log))).length;
	}

	buildEvent (gameboard, log) {

		return { src: gameboard.find(log.src), data: log.data.map(d => { let x = gameboard.find(d); return x ? x : d; } ) };
	}
}
