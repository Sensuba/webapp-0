...
export default (state, n, callback) => {
  ...
  switch(n.type) {
    ...
    case "newcard": {
      ...
      let card = state.find(n.data[0].id);
      if (card)
        waiter = card.identify(n.data[0]);
      break;
    } case "transform":
  ...
  if (waiter) waiter(() => callback({type: n.type, src:n.src, data:n.data, state:state}));
  else callback({type: n.type, src:n.src, data:n.data, state:state});
}
